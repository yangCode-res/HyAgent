...
if __name__ == "__main__":
    ...
    test_content = """
    # Introduction
    Hello world! This is a test paragraph. It contains English words. (Count: ~10 words)
    
    这是第二段。这里是中文内容。它应该按照字符数来统计。(统计: ~20 chars)
    
    Mixed content paragraph: 这里的字数是 mixed 的。The algorithm should handle this correctly.
    """
    path="../utils/ocr_md_outputs/ocr_result_1.md"
    print(len(split_md_by_mixed_count(path)["ocr_result_1"]))
    # 模拟 counting 测试
    # print(f"段落1计数: {get_mixed_word_count('Hello world! This is a test paragraph. It contains English words.')}") 
    # 预期: 10左右
    
    # print(f"段落2计数: {get_mixed_word_count('这是第二段。这里是中文内容。')}") 
    # 预期: 13 (中文及标点)
    
    # print(f"段落3计数: {get_mixed_word_count('Mixed content: 这里的字数。')}") 
    # 预期: 2 (Mixed, content) + 6 (这里的字数。) = 8